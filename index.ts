...
type validPhrases =
	" " | "ms" | "s" | "m" | "h" | "milisecond" | "miliseconds" | "second" | "seconds" | "minute" | "minutes" | "hour" | "hours";
...
class Timer {
	...
	getReadable(miliseconds, options: IModuleOptions = { brief: this.brief }) {
		...
		if (seconds < 1) {
			return `${parseFloat(miliseconds.toFixed(2))}${
				brief ? translate('ms') : miliseconds === 1 ? translate('milisecond', useSpace) : translate('miliseconds', useSpace)
				}`
		}
	...
	elapsed(options: IElapsedOptions = { start: this.start, end: Date.now(), brief: this.brief }) {
		return this.getReadable(this.elapsedRaw(options), { brief: (options.brief || this.brief) })
	}

	elapsedVerbose(options: IElapsedOptions = { start: this.start, end: Date.now() }) {
		return this.getReadable(this.elapsedRaw(options), { brief: false })
	}

	elapsedBrief(options: IElapsedOptions = { start: this.start, end: Date.now() }) {
		return this.getReadable(this.elapsedRaw(options), { brief: true })
	}

	elapsedRaw(options: IElapsedOptions = { start: this.start, end: Date.now() }) {
		return (options.end || Date.now()) - (options.start || this.start)
	}

	translate(phrase: validPhrases, useSpace = this.brief) {
		const _ = useSpace ? lang[this.language].briefSpace ? " " : "" : lang[this.language].verboseSpace ? " " : "";
